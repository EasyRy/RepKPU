...
def load_kernels(radius, num_kpoints, num_kernels, dimension, fixed):
    ...
    # Kernel directory
    kernel_dir = join(DIR, "dispositions")
...
if __name__ == '__main__':
    a = load_kernels(radius=1, num_kpoints=17, num_kernels=5, dimension=3, fixed='center')
    
    for _ in range(len(a)):
        print(_)
        for i in range(1, len(a[_])):
            print(np.sum((a[_,i]-a[_,0])**2))
    # for i in 
"""